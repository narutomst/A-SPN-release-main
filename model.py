...
from tensorflow.keras.layers import InputLayer, Dense, Reshape, Lambda, Dropout
...
def get_model(img_rows, img_cols, num_PC, nb_classes, dataID=1, type='aspn', lr=0.01):
    ...
    # metrics = [tf.keras.metrics.SparseCategoricalAccuracy(name="my_accuracy")]
    # metrics = tf.keras.metrics.SparseCategoricalAccuracy(name="my_accuracy")
    # metrics = ['acc', 'accuracy']
    # metrics = ['accuracy']
    model.compile(optimizer=rmsp, loss=loss, metrics=metrics)
    return model
...
def spn(img_rows, img_cols, num_PC, nb_classes):
    CNNInput = tf.keras.Input(shape=(img_rows, img_cols, num_PC), name='i0')
...
def aspn(img_rows, img_cols, num_PC, nb_classes):
    ...
    F = Reshape([img_rows * img_cols, num_PC])(CNNInput)  # tf.keras.layers.Reshape(target_shape, **kwargs)
    F = BatchNormalization()(F)
    F = Dropout(rate=0.5)(F)
